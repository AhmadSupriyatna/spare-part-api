...
- **2026-09-15** (lanjut di PC rumah, setelah pull hasil kerja kantor) — User bertanya soal kegunaan BOM, lalu menilai section itu redundan dengan panel "Part" di halaman Line/Mesin/Equipment yang baru dibangun di kantor (yang membaca `part_installations`, bukan BOM). Keputusan: **BOM (`equipment_parts`) dihapus total**, dan fitur yang bergantung padanya dibenahi supaya membaca relasi part↔equipment dari instalasi nyata, bukan spesifikasi terpisah yang sering tidak lengkap diisi.
  - **Backend**: migration baru `drop_equipment_parts_table` (down() mengembalikan skema asli kalau perlu rollback); model `EquipmentPart`, `EquipmentPartController`, `EquipmentPartResource`, `StoreEquipmentPartRequest` dihapus; relasi `equipmentParts()` di `Part`/`Equipment` dihapus; rute `GET/POST /equipment/{equipment}/parts`, `GET /parts/{part}/equipment`, `DELETE /equipment-parts/{id}` dihapus.
    - `PublicBreakdownController::equipmentForPartInBranch()` (dipakai scan QR breakdown) dirombak: sekarang query `Equipment::whereHas('partInstallations', ...)` (part terpasang aktif) bukan `$part->equipmentParts()`.
    - `StoreReplacementRequestRequest`: validasi server-side `equipment_id` yang tadinya cek `equipment_parts` sekarang cek `part_installations` (part harus benar-benar terpasang aktif di equipment itu).
    - `TaskService::generateNextTask()`: default kuantitas part dari BOM (`equipment_parts.quantity_required`) dihapus — teknisi isi manual saat menyelesaikan tugas (sudah didukung alur `part_stock_id`/`quantity_used` yang ada).
    - `LifecycleDemoSeeder`: blok seeding BOM dihapus; data demo breakdown request sekarang pilih part dari instalasi aktif (`PartInstallation`) di equipment utama, bukan dari BOM.
  - **Frontend**: folder `features/equipment-parts/` dihapus; section "Bill of Material (BOM)" di `EquipmentDetailPage` dan tab "Digunakan di Equipment" di `PartDetailPage` dihapus beserta query/mutation pendukungnya; tipe `EquipmentPart` dihapus dari `types/relations.ts`.
  - Diverifikasi: `migrate:fresh --seed` (termasuk `LifecycleDemoSeeder`) sukses tanpa BOM; scan QR breakdown untuk part yang terpasang di equipment tertentu mengembalikan equipment yang benar (dan kosong untuk cabang lain / equipment yang tidak memakainya); submit permintaan penggantian untuk kombinasi part+equipment yang tidak terpasang ditolak (422) dengan pesan baru; `npm run build` & lint bersih 0 error di kedua repo.
